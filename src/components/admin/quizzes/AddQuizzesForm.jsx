...
import QuizInfo from './stepper/QuizInfo';
import QuizOptions from './stepper/QuizOptions';
...
const AddQuizzesForm = ({ setMode }) => {
    ...
    const handleAdd = (e) => {
        e.preventDefault();
        if (!formData.options.every(option => option.option !== "")) {
            setError("Options can not be blank string!")
        } else if (!formData.options.some(option => option.isCorrect === true)) {
            setError("choose any of the correct answer")
        } else {
            setError("");
